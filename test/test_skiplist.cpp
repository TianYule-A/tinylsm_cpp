...
TEST(SkipListTest, MemorySizeTracking) {
  ...
  // 验证内存大小
  size_t expected_size = sizeof("key1") - 1 + sizeof("value1") - 1 + sizeof(uint64_t) + sizeof("key2") - 1 +
...
TEST(SkipListTest, IteratorPreffix) {
  ...
  EXPECT_EQ(skipList.begin_preffix("not exist"), skipList.end_preffix("not exist"));
}
...
TEST(SkipListTest, ItersPredicate_Base) {
  ...
  // 测试前缀匹配
  auto prefix_result = skipList.iters_monotony_predicate([](const std::string &key) {
  ...
  auto range_result = skipList.iters_monotony_predicate([&range](const std::string &key) {
    if (key < range.first) {
      return 1;
...
TEST(SkipListTest, TransactionId) {
  if (CURRENT_LAB < 5.1) {
    GTEST_SKIP() << "Skipping transaction id test until Lab 5.1 is implemented";
  }
  SkipList skipList;
