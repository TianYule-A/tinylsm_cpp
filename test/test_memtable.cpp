...
TEST(MemTableTest, ItersPredicate_Base) {
  ...
  // 测试前缀匹配
  auto prefix_result = memtable.iters_monotony_predicate(0, [](const std::string &key) {
  ...
  auto range_result = memtable.iters_monotony_predicate(0, [&range](const std::string &key) {
    if (key < range.first) {
      return 1;
