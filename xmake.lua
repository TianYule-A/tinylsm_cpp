...
add_options("clang_tidy", {  -- 添加 clang-tidy 选项
    default = true ,
    target = "all",  -- 适用于所有目标
    description = "Enable clang-tidy for static code analysis"
})
...
--     set_policy("build.sanitizer.address", true)  -- 启用AddressSanitizer
-- end
...
-- 定义输出目录
function set_my_target_dir(base_target_dir)
    after_load(function (target)
        if is_mode("debug") then
            target:set("targetdir", base_target_dir .. "/debug")
            target:set("symbols", "debug")
            target:set("strip", "none")
            target:set("optimize", "none")
        elseif is_mode("release") then
            target:set("targetdir", base_target_dir .. "/release")
            target:set("symbols", "debug")
...
-- 定义benchmark，用于性能分析
target("benchmark_skiplist")
    set_kind("binary")
    set_group("benchmark")
    add_files("benchmark/benchmark_skiplist.cpp")
    add_deps("skiplist")
    add_packages("toml11", "spdlog")
    add_includedirs("include")
    add_syslinks("profiler")  -- 添加 profiler 库链接
    add_cxxflags("-lprofiler")
    set_my_target_dir("$(buildir)/benchmark")  -- 设置输出目录
    add_options("clang_format")
    if is_mode("debug") then
        set_symbols("debug")
        set_optimize("none")
        set_strip("none") -- <-- 确保它在这里，并且没有被其他东西覆盖
    elseif is_mode("release") then
        set_symbols("debug") -- 即使是 release 也要调试符号
        set_optimize("fast")
        set_strip("none")    -- <-- 即使在 release 模式下也必须设置为 none
    end
