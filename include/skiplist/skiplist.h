...
struct SkipListNode {
  ...
  std::vector<std::weak_ptr<SkipListNode>> backward_;   // 指向不同层级的下一个节点的指针数组
...
class SkipListIterator : public BaseIterator {
 ...
 private:
  ...
  std::shared_ptr<std::shared_lock<std::shared_mutex>> lock;  // 持有读锁, 整个迭代器有效期间都持有读锁, memtable的锁用于保护整个跳表的读写操作，迭代器读锁用于保护迭代器范围元素的访问
};
...
class SkipList {
 private:
  ...
  std::uniform_int_distribution<> dis_01;  // 均匀整数分布
 ...
 public:
  ...
  /**
   * @brief 查询满足谓词的一段连续区间
   * @param predicate 谓词函数， 返回值：
      =0：满足谓词
      >0: 不满足谓词, 需要向右移动
      <0: 不满足谓词, 需要向左移动
   * @return 返回一个包含起始和结束迭代器的可选值
   */
  std::optional<std::pair<SkipListIterator, SkipListIterator>> iters_monotony_predicate(
